...
class TextConfig_2:
    text_size: int = 147
    image_size: int = 128
    block_size: int = 275  # max sequence length
...
class CasualSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        casual_mask = torch.tril(torch.ones(config.text_size, config.text_size))
        full_mask = torch.ones(
            config.image_size + config.text_size, config.image_size + config.text_size
        )
        full_mask[-config.text_size :, -config.text_size :] = casual_mask
        self.register_buffer(
            "bias",
            full_mask.view(
                1,
                1,
                config.image_size + config.text_size,
                config.image_size + config.text_size,
            ),
        )
...
class Text_generation(nn.Module):
    def __init__(self, config):
        ...
        self.to_out = nn.Linear(config.n_embd, config.vocab_size, bias=False)
